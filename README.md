...
- While Qwen is selected, STM prewarms it at app launch, starts the microphone/HUD before issuing a single-token model touch for each recording, and runs the same heartbeat every 15 seconds after 10 seconds idle. The heartbeat timer starts only after the model reports ready. If the helper exits, STM restarts and prewarms it automatically; an in-flight transcription retries once after transport failure. Switching to Cloudflare stops the helper and releases its roughly 1.8GB working set.
- The first local use after STM itself launches may overlap the one-time 20–30 second model preload; this is the only expected cold start. After the `qwen local runtime ready` event, recording startup is microphone-first and idle heartbeats preserve the warm state.
- Qwen recordings longer than 12 seconds now begin live transcription before recording stops. STM waits for at least 150 ms below -38 dBFS between 12 and 16 seconds, forces a boundary at 16 seconds if speech continues, normalizes and transcribes each completed segment sequentially through the retained helper, and joins results in recording order. Dictations shorter than 12 seconds keep the exact whole-file path.
- Qwen generation is bounded by recording duration so short or noisy clips cannot wander through an unbounded autoregressive decode.
